...
void CrealityPrintHostSendDialog::init()
{
    ...
    auto* group_box = new wxStaticBox(this, wxID_ANY,
        _L("Printer: ") + from_u8(printer_name));
    ...
    if (gcode_filament_count > 0 && !m_printer_slots.empty()) {
        ...
        for (int i = 0; i < gcode_filament_count; i++) {
            ...
            auto* type_label = new wxStaticText(this, wxID_ANY,
                wxString::Format("%d", i + 1) + wxString(" (") + from_u8(gc_type) + wxString(")"));
            ...
            for (auto& slot : m_printer_slots) {
                ...
                if (slot.box_id == 0) {
                    label_str = _L("Ext");
                    if (!slot.type.empty())
                        label_str += wxString(" - ") + from_u8(slot.type);
                } else {
                    label_str = from_u8(slot.tool_id.size() > 1 ? slot.tool_id.substr(1) : slot.tool_id);
                    if (!slot.type.empty())
                        label_str += wxString(" - ") + from_u8(slot.type);
                }
        ...
        warning->SetForegroundColour(StateColor::darkModeColorFor(wxColour("#323A3D")));
        group_sizer->Add(warning);
    }
...
std::map<std::string, std::string> CrealityPrintHostSendDialog::extendedInfo() const
{
    ...
    for (int i = 0; i < (int)m_slot_combos.size(); i++) {
        ...
        if (sel >= 0 && sel < (int)m_printer_slots.size()) {
            ...
            // not the destination CFS slot ΓÇö firmware matches by gcode tool.
